...
Run `setup_torch()`, `setup_java()`, or `check_r5r()` to configure
...
See `vignette("introduction")` for a full walkthrough:
[Getting Started with interpElections](https://antrologos.github.io/interpElections/articles/introduction.html).
...
See `vignette("brazilian-elections")` for the complete workflow:
[Interpolating Brazilian Electoral Data](https://antrologos.github.io/interpElections/articles/brazilian-elections.html).
...
See `vignette("custom-data")` for a self-contained example with synthetic
data: [Using interpElections with Your Own
...
where *t* is the travel time in minutes and *&alpha;* is a per-zone
decay parameter. The weight matrix is then **column-standardized** so
that each source distributes exactly 100% of its data across the
tracts --- guaranteeing total conservation.

The optimal *&alpha;* vector is found by minimizing the squared error
between interpolated and observed demographic totals:

$$f(\alpha) = \sum_{i,k} \bigl(\hat{V}_{ik} - P_{ik}\bigr)^2$$

where *V&#x0302;* = *W*<sup>std</sup>(*&alpha;*) &times; *V* (interpolated values)
and *P* is the known population matrix. An **analytical gradient** makes
this feasible for thousands of zones.

See `vignette("understanding-the-model")` for the full mathematical
treatment: [Understanding the IDW Interpolation
Model](https://antrologos.github.io/interpElections/articles/understanding-the-model.html).

## Vignettes

interpElections ships with six vignettes that form a progressive tutorial:
...
| Function | Purpose |
|---|---|
| `idw_weights()` | Build column-standardized weight matrix from travel times and alpha |
| `idw_interpolate()` | Apply IDW weights to transfer source data into target zones |
| `idw_objective()` | Evaluate sum-of-squared-errors loss at a given alpha |
| `idw_gradient()` | Analytical gradient of the objective (for L-BFGS-B) |
| `optimize_alpha()` | Find optimal per-zone alpha via GPU (ADAM) or CPU (L-BFGS-B) |

### High-level wrappers

| Function | Purpose |
|---|---|
| `interpolate_election()` | One-call interpolation from sf points to sf polygons |
| `interpolate_election_br()` | Full Brazilian pipeline with auto-download |

### Brazilian data helpers

| Function | Purpose |
|---|---|
| `br_prepare_population()` | Census population by age bracket per tract |
| `br_prepare_tracts()` | Census tract geometries with population data |
| `br_prepare_electoral()` | TSE electoral data aggregated to polling locations |
| `br_download_votes()` | Raw candidate vote counts from TSE |
| `br_download_turnout()` | Turnout and abstention data from TSE |
| `br_download_geocode()` | Polling station coordinates from TSE |
| `compute_travel_times()` | Travel-time matrix via r5r routing |
| `download_r5r_data()` | OSM road network download for r5r |

### Areal aggregation

| Function | Purpose |
|---|---|
| `areal_weights()` | Area-weighted intersection matrix between polygon layers |
| `areal_interpolate()` | Aggregate interpolated data into larger zones |

### Setup and diagnostics

| Function | Purpose |
|---|---|
| `setup_torch()` / `check_torch()` | Install and verify GPU (torch) support |
| `setup_java()` / `check_r5r()` | Install and verify Java/r5r for travel times |
| `setup_osmium()` | Install osmium for OSM data clipping |
| `use_gpu()` | Toggle GPU acceleration on/off |

### Cache management

| Function | Purpose |
|---|---|
| `get_interpElections_cache_dir()` | Show current cache location |
| `set_interpElections_cache_dir()` | Change cache location |
| `interpElections_cache()` | List cached files and sizes |
| `interpElections_cache_clean()` | Delete cached data by category |

## GPU acceleration

For municipalities with more than ~1,000 census tracts, GPU
acceleration provides significant speedups. The package supports NVIDIA
GPUs (CUDA) and Apple Silicon (MPS):
...
See `vignette("performance-and-configuration")` for details:
