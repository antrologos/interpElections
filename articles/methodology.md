...
The `min_area_for_pop_weight` parameter (default: 1 km²) controls the
threshold: tracts smaller than this use `point_on_surface` regardless of
the chosen method, since small urban tracts are dense enough that any
interior point is a reasonable proxy.
...
- **Max trip duration**: 180 minutes (default); pairs beyond this
  threshold are `NA` (unreachable) and receive exactly zero weight
...
votes, the interpolation must assign exactly 500 votes across all tracts
— no more, no less.
...
$$W_{ij} = \frac{K_{ij}}{\sum\limits_{i}K_{ij}}$$

where $K_{ij} = \left( t_{ij} + 1 \right)^{- \alpha_{i}}$ is the IDW
kernel from Section 10.
...
With **aggregate** column normalization, the weight matrix has one set
of weights for all age brackets. The optimizer must find one set of
weights that simultaneously matches multiple different demographic
distributions. This is an inherent compromise: weights that correctly
distribute 18–20 year-olds may systematically misallocate 60–69
...
1.  **Compute the IDW kernel**:
    $K_{ij}^{(k)} = \left( t_{ij} + 1 \right)^{- \alpha_{i}}$ (same
    kernel for all brackets, since alpha is shared)
2.  **Column-normalize**:
    $W_{ij}^{(k)} = K_{ij}^{(k)}/\sum_{i}K_{ij}^{(k)}$

**Aggregate:**

$$W_{\text{total}} = \frac{1}{K}\sum\limits_{k = 1}^{K}W^{(k)}$$
...
function to separately account for each age group’s spatial pattern.
...
#> Column-normalized: 141 70 9 28 12
...
#> Column-normalized: 113 291 44 203 90
...
[`optimize_alpha()`](https://antrologos.github.io/interpElections/reference/optimize_alpha.md)
as `result$W`. It can also be recomputed from pre-existing alpha values
using
[`compute_weight_matrix()`](https://antrologos.github.io/interpElections/reference/compute_weight_matrix.md):
...
$\left. \alpha\rightarrow\text{kernel}\rightarrow\text{column-normalize}\rightarrow\text{interpolation}\rightarrow\text{error} \right.$.
...
forward pass (kernel -\> column-normalize -\> interpolation -\> loss),
and torch computes the gradient of the loss with respect to $\alpha$
...
becomes exactly zero, the column normalization produces
$0/0 = \text{NaN}$, and the optimization collapses.

**The log-domain solution**: the IDW kernel is computed in log space as
$\log K_{ij} = - \alpha_{i} \cdot \log\left( t_{ij} + 1 \right)$, and
the column normalization uses the **logsumexp** trick for numerical
stability. The logsumexp function
$\log\left( \sum_{j}\exp\left( x_{j} \right) \right)$ is a numerically
stable primitive available in torch.
...
| `log_K` | $\lbrack n \times m\rbrack$                          | Log-kernel (broadcast to 3D)             |
| `W_all` | $\left\lbrack k_{a} \times n \times m \right\rbrack$ | Per-bracket weight tensors               |

The first dimension ($k_{a}$) indexes demographic brackets. All $k_{a}$
...
optimizer, with **per-bracket column normalization**. At each gradient
step:

1.  **Build the IDW kernel** from current alpha values
2.  **Per-bracket column normalization**: each demographic column uses
    the same column-normalized kernel
3.  **Compute loss**: SSE between interpolated and observed values
4.  **Backward + Adam step** on the unconstrained `theta` parameters

At the end of each epoch, the **true objective** is evaluated on the
full dataset (no gradient) and used for convergence monitoring and
logging.
...
The epoch loss curve shows the **true objective** evaluated on all
tracts at the end of each epoch. The package tracks the best epoch loss
and returns the corresponding alpha values.

**Key tuning parameters**:

Tuning parameters are grouped in
...
The residual boxplots confirm the pattern across all four cities: IDW
...
In every city, the IDW method achieves dramatically lower squared error
than Voronoi assignment — by a factor of 275x (Cuiaba) to 1,624x
...
1.  **Full kernel matrix** (always in memory):
    $n \times m \times \text{bytes}$ — e.g., 5,100 x 407 x 4 = 8 MB for
    Belo Horizonte in float32.
2.  **Per-bracket weight tensors**:
    $k_{a} \times n \times m \times \text{bytes}$ — torch must store
    intermediate tensors for backpropagation through the column
    normalization.

where $k_{a} = 14$ (gender × 7 age brackets) and `bytes` is 4 (float32)
or 8 (float64).
...
For very large cities on GPU, consider using `dtype = "float32"` (the
default) rather than `"float64"` to halve memory usage.
...
- $S \in {\mathbb{R}}^{M \times K}$: voter counts by age bracket at
  stations
- $c \in {\mathbb{R}}^{M}$: column targets ($c_{j} = 1$, enforced by
...
$$W_{ij} = \frac{K_{ij}}{\sum\limits_{i\prime}K_{i\prime j}}$$

In log domain (for numerical stability):

$$W_{ij} = \exp\!\left( \log K_{ij} - \text{logsumexp}_{i}\left( \log K_{ij} \right) \right)$$

#### Per-bracket column normalization

For each demographic bracket $k = 1,\ldots,K$:

1.  Compute kernel: $K_{ij} = \left( t_{ij} + 1 \right)^{- \alpha_{i}}$
2.  Column-normalize:
    $W_{ij}^{(k)} = K_{ij}/\sum_{i\prime}K_{i\prime j}$

Aggregate and column-normalize:
$$W_{\text{total}} = \sum\limits_{k = 1}^{K}W^{(k)}$$$$W_{ij} = W_{\text{total},ij}/\sum\limits_{i\prime}W_{\text{total},i\prime j}$$
...
$$\nabla_{\alpha}f = \frac{\partial}{\partial\alpha} \parallel \text{colnorm}\left( \text{kernel}(\alpha) \right) \cdot S - P \parallel_{F}^{2}$$
