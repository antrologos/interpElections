...
Additionally, all years produce gender × age columns for the same
voting-age brackets, split by gender: `pop_hom_alf_*`, `pop_hom_nalf_*`,
`pop_mul_alf_*`, `pop_mul_nalf_*`. These are used by the "full"
calibration mode in
[`interpolate_election_br()`](https://antrologos.github.io/interpElections/reference/interpolate_election_br.md),
which sums literate + illiterate within each gender × age group.
