...
use pre-computed results from **Niterói (RJ)** (1,169 tracts, GPU) and
**Belo Horizonte (MG)** (5,113 tracts, GPU).
...
observed) for each tract and age bracket.
...
`residuals()` and `reinterpolate()`). Use `keep` for additional objects:
