...
result <- structure(list(
  ...
  calib_cols   = list(
    tracts  = c("pop_18_34", "pop_35_54", "pop_55_69"),
...
- **Census tracts / Sources / Variables** -- dimensions of the interpolation.
...
plot(result, "Candidate_A", type = "absolute")
```
...
```{r extract-alpha}
alpha <- coef(result)
length(alpha)  # one per census tract
summary(alpha)
...
- **High alpha (e.g., > 5)**: Weights drop off steeply with distance.
  The tract's population is almost entirely allocated to the nearest
  source. This occurs in areas where the closest polling location
  dominates.

- **Low alpha (e.g., < 1)**: Weights decay slowly. Many sources
  contribute to the tract. This occurs in densely served areas where
...
3. **Alpha distribution.** Look for extreme outliers. Alphas at the upper
   bound (default 20) suggest the census tract is effectively assigned to a single
