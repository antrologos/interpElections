...
spatial allocation: the package finds per-census-tract decay parameters that minimize
...
plot(result, "budget", type = "absolute")
```
...
`n_tracts x m_points`. Values must be positive (they represent distances or
...
- Rows correspond to target polygons (same order as `tracts_sf`).
...
`optimize_alpha()` finds per-census-tract decay parameters that minimize the squared
...
into the target polygons. The alpha vector is reusable across variables -- you
