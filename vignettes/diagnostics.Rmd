...
where $t_{ij}$ is the travel time (in minutes) from tract $i$ to station $j$. The final weight is $W_{ij} = K_{ij} / \sum_i K_{ij}$ (column-normalized so each station's voters are fully distributed). See `vignette("methodology")` for details on kernel choice.
